...
\begin{enumerate}[nolistsep]
  ...
  \item \textbf{Face Recognition Pipeline} --
    \begin{itemize}[nolistsep]
      \item RetinaFace-500MF ONNX (\texttt{det\_500m.onnx}) untuk deteksi wajah dalam frame kamera
      \item Face Landmarks untuk ekstraksi landmark mata (EAR)
      \item Liveness Detection via Eye Aspect Ratio (EAR) -- mendeteksi kedipan alami sebagai anti-spoofing
      \item InsightFace ONNX (\texttt{w600k\_mbf.onnx}) untuk ekstraksi embedding 512-dimensi
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{lXl}
    ...
    Face Detection & RetinaFace-500MF ONNX & Deteksi wajah dalam frame \\
    Face Embedding & InsightFace ONNX (MobileFaceNet) & Ekstraksi vektor wajah 512-d \\
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[
    ...
    \matrix[row sep=0.4cm, column sep=0.4cm] {
      ...
      \node[pipe] (detect) {RetinaFace\\ONNX Detection}; &
      \node[pipe] (landmark) {Face\\Landmarks}; &
      \node[pipe] (liveness) {Liveness EAR\\Blink Detection}; \\
      \node[pipe] (embed) {InsightFace MBF\\ONNX Embedding}; &
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{lXcc}
    ...
    \midrule
    Deteksi Wajah & RetinaFace-500MF ONNX & $\sim$2.4 MB & $<$ 5ms \\
    Landmark Wajah & Face Landmarks & Bundle & $<$ 2ms \\
    Liveness & Eye Aspect Ratio (rule-based) & 0 KB & $<$ 2ms \\
    Embedding & InsightFace .onnx (w600k\_mbf) & $\sim$13.6 MB & $\sim$15ms \\
    Matching & Brute-force cosine similarity & $\sim$20 MB (10k faces, 512-d) & $\sim$3ms \\
    \midrule
    \textbf{Total} & & \textbf{$\sim$16--17 MB} & \textbf{$\sim$25ms} \\
    \bottomrule
  \end{tabularx}
\end{table}
